...
export default function SpellingScreen({ navigation }: Props) {
  ...
  const overallProgress = questionsRef.current.length;

  const handleSubmit = useCallback(() => {
    if (!answer.trim() || !currentLetter) return;
    ...
    showFeedback(isCorrect, correctNatoWord);
    setAnswer('');
  ...
  }, [answer, currentLetter, correctNatoWord, letterIndex, word, wordsCompleted, correctCount, recordAnswer, hapticEnabled, soundEnabled, correctPlayer, wrongPlayer, showFeedback, recordSessionComplete, checkAchievements]);
          ...
          <View style={styles.cardArea}>
            ...
            <View style={styles.wordRow}>
              {word.split('').map((char, i) => {
                ...
                const isFlashTarget = isActive && flashColor;
                return (
                  <View
                    key={i}
                    style={[
                      styles.letterBox,
                      isActive && !isFlashTarget && styles.letterBoxActive,
                      isDone && styles.letterBoxDone,
                      isFlashTarget && flashColor === 'success' && styles.letterBoxFlashSuccess,
                      isFlashTarget && flashColor === 'error' && styles.letterBoxFlashError,
                    ]}
                  >
                    <Text style={[
                      styles.letterBoxText,
                      isActive && styles.letterBoxTextActive,
                      isDone && styles.letterBoxTextDone,
                    ]}>
                      {char}
                    </Text>
                  </View>
            ...
            <Text style={styles.hint}>Word {wordsCompleted + 1} of {wordsQueue.current.length}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  letterBox: {
    ...
    backgroundColor: COLORS.backgroundCard, borderWidth: 2, borderColor: COLORS.cardBorder,
    alignItems: 'center', justifyContent: 'center',
  },
  letterBoxActive: { borderColor: COLORS.primary, backgroundColor: 'rgba(124,58,237,0.15)' },
  letterBoxDone: { borderColor: COLORS.success, backgroundColor: 'rgba(16,185,129,0.1)' },
  letterBoxFlashSuccess: { borderColor: COLORS.success, backgroundColor: 'rgba(16,185,129,0.25)' },
  letterBoxFlashError: { borderColor: COLORS.error, backgroundColor: 'rgba(239,68,68,0.25)' },
